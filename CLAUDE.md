...
| `tenders.html` | 해외 계량기(전기/가스/수도) 입찰 트래커 — 인도부터 시작, 동남아·서남아·중국·유럽으로 확대 | https://sgspeed04.github.io/Tracker/tenders.html |
...
- **국가별 바로가기**: 인도 중앙(GeM/CPPP)·인도 주정부/DISCOM(마하라슈트라 MahaTenders·MSEDCL·마디아프라데시·타밀나두·우타르프라데시·라자스탄·구자라트 nProcure)·인도 가스 배급사(GAIL·IGL·MGL)·인도 상수도청(Delhi Jal Board·UP Jal Nigam)·인도네시아 중앙(INAPROC)·인도네시아 전력(PLN)·인도네시아 가스(PGN)·인도네시아 상수도(PAM Jaya)·베트남(VNEPS)·태국 중앙(e-GP)·태국 전력(MEA 방콕·PEA 지방)·태국 가스(PTT)·태국 상수도(MWA 방콕·PWA 지방)·필리핀(PhilGEPS)·필리핀 상수도(LWUA)·방글라데시(e-GP)·파키스탄(PPRA)·중국(CCGP)·**유럽**(EU 전역 TED, 프랑스 BOAMP/PLACE, 영국 Find a Tender, 독일 DTVP, 이탈리아 Acquisti in Rete, 스페인 PLACSP, 네덜란드 TenderNed, 폴란드 e-Zamówienia) 조달포털 홈페이지 링크 + Google 사이트 검색 바로가기(추천 키워드 포함) — 전기·가스·수도를 각각 별도 기관(국영 전력공사/가스공사, 지역 상수도)이 운영하는 나라는 유형별 전담 포털을 추가함(인도, 인도네시아, 태국). 인도네시아 상수도는 PDAM이 도시별로 분산 운영돼 자카르타 PAM Jaya를 대표 사례로 걸어둠 — 다른 지역은 자체 PDAM 포털을 별도로 확인해야 함. 태국은 전력·상수도 모두 방콕 대도시권(MEA/MWA)과 나머지 지방(PEA/PWA)을 서로 다른 국영기관이 담당하는 이원 구조라 각각 두 기관씩 추가함. 베트남은 반대로 입찰 관련 법령상 EVN(전력)·PV Gas·지역 상수도(SAWACO/HAWACO 등) 국영기업도 전부 국가 전자조달망(VNEPS) 한 곳에 공시해야 해서 기관별 포털을 따로 추가하지 않음(검색 중 확인됨 — 개별 기관 전용 조달 포털은 못 찾음). 필리핀은 전력이 NEA 산하 121개 지역 전력협동조합으로 쪼개져 있어 통합 포털 없이 대부분 PhilGEPS에 개별 공고되고, 가스는 배관망 인프라가 미미해(LPG 위주) 계량기 입찰 자체가 드물어 중앙(PhilGEPS)에 이 배경 설명만 추가하고 별도 전력/가스 기관 링크는 넣지 않음 — 상수도는 메트로마닐라 외 지역 수도청을 감독하는 LWUA의 입찰 페이지를 확인해 추가함. 유럽은 인도·동남아와 성격이 다름 — EU 임계값 이상 공고는 TED에 의무 게재되지만 실제 응찰은 MID(계량기기지침) 강제 인증(자율선언 불가)·로컬 파트너·사이버복원력법(CRA) 대응이 선행돼야 해서 조달포털 모니터링만으로 진입하기 어려움. `국가별 바로가기`에는 8개국 포털을 추가했지만 완제품 입찰 리드 소싱보다는 시장 리서치·부품 서플라이체인 진입 참고용에 가깝다는 점을 tCountry 드롭다운(프랑스/영국/독일/이탈리아/스페인/폴란드/네덜란드 추가)과 함께 반영함. 상세 배경은 별도 작성한 "유럽 계량기 시장 보고서"(규제 타임라인, 국가별 보급률, 경쟁 구도, CPV 코드, 진입장벽) 참고. 수동 검색 후 발견한 입찰을 바로 등록하는 방식이 현재 가장 안정적
- **자동수집 후보 탭(다국가 파일럿)**: `scripts/fetch-tenders.js`가 `.github/workflows/update-tenders.yml`을 통해 매일 05:00 KST 자동 실행, 국가별 대표 포털에서 "최신 공고" 목록을 긁어 제목에 계량기 관련 키워드가 있는 것만 걸러 `data/tender_candidates.json`에 저장
  - 대상: 인도(CPPP·GeM), 베트남(VNEPS), 태국(PWA), 필리핀(PhilGEPS), 인도네시아(INAPROC), 방글라데시(e-GP), 파키스탄(PPRA), 중국(CCGP, 电表/燃气表/水表 키워드)
  - 인도 CPPP는 실제 GitHub Actions 실행 로그로 동작이 검증됨(`table.list_table tbody tr` 선택자가 실제로 맞음). GeM은 `net::ERR_CONNECTION_REFUSED`로 차단 확인됨(데이터센터 IP 차단 WAF로 추정, PropertyLeads의 VWorld/Azure IP 차단과 동일 패턴). 나머지 신규 대상들은 이 세션에서 사이트 접근 자체가 막혀 실제 HTML을 못 보고 선택자를 방어적으로 작성함 — 국가별로 실제 성공 여부는 Actions 로그에서 개별 확인 필요(대상마다 성공/실패가 다를 수 있음)
  - 실패한 대상은 조용히 스킵되고 기존 데이터를 덮어쓰지 않음 — 일부 국가가 막혀 있어도 나머지 대상 수집과 수동 등록 워크플로에는 영향 없음
  - 확장 시 `scripts/fetch-tenders.js`의 `TARGETS` 배열에 `{country, source, url, keywords}` 항목 추가
