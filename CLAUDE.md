...
- **자동수집 공고 탭**: 강남구청 공시송달 게시판 파일럿 스크래핑 결과(`data/violations_notices.json`)를 보여주고, 바로 리드로 등록하거나 숨길 수 있음
  - `scripts/fetch-violations.js`가 `.github/workflows/update-violations.yml`을 통해 매일 00:30 KST 자동 실행
  - 위반건축물 관련 키워드(철거명령/이행강제금/무단증축 등)로 제목을 필터링 — 강남구청 게시판이 다른 기관 공고도 섞어 올리기 때문
